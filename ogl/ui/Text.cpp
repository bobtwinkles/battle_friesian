...
//static fri::ogl::ShaderProgram * text_shader;
//static fri::ogl::ShaderProgram * save;
...
using namespace fri::ogl::ui;

#define CACHE_TEXTURE_SIZE 128

void fri::ogl::ui::TextInit() {
  FcResult result;
  char * file;
  int error;
  ...
  FcPatternAddBool(pattern, "scalable", true);
  ...
  if (fs) {
    for (int j = 0; j < fs->nfont; ++j) {
      if (FcPatternGetString(fs->fonts[j], FC_FILE, 0, (FcChar8 **)&file) != FcResultMatch) {
        ...
        if (strstr(file, "Mathematica") != NULL) {
          std::cerr << "Blacklisting mathematica fonts" << std::endl;
          continue;
        }
        if (FT_New_Face(library, (const char *)file, 0, &freetype_font)) {
          ...
          if (freetype_font->num_glyphs > 100) {
            if (FT_Load_Char(freetype_font, 'A', FT_LOAD_RENDER)) {
              std::cerr << "Failed to load an example character, will use a different font" << std::endl;
              continue;
            }
...
void fri::ogl::ui::TextDrawBegin(const fri::ogl::Environment & Env) {
  rendering = true;

  TextDrawColor(Env, 1, 1, 1);
}

void fri::ogl::ui::TextDrawColor(const fri::ogl::Environment & Env, float R, float G, float B) {
  if (!rendering) {
    std::cerr << "tried to set text color outside of text rending mode!" << std::endl
              << "You should call TextDrawBegin/TextDrawEnd" << std::endl;
    BUG();
  }
  Env.GetCurrentShaderProgram()->Upload("color", glm::vec4(R, G, B, 1));
}

void fri::ogl::ui::TextDrawString(const fri::ogl::Environment & Env, float X, float Y, const char * String) {
  glm::mat4 trans;
  const char * p;
  fri::ogl::ShaderProgram * shader = Env.GetCurrentShaderProgram();
  ...
  glBindTexture(GL_TEXTURE_2D, texture);
  shader->Upload(NAME_TEXTURE, 0);

  glEnableVertexAttribArray(LOC_COORDINATE);
  glVertexAttribPointer(LOC_COORDINATE, 4, GL_FLOAT, false, 4 * sizeof(GLfloat), 0);
...
void fri::ogl::ui::TextDrawEnd(const fri::ogl::Environment & Env) {
  GLERR();
  rendering = false;
}
